...
def plot_modelo_3d(alturas, elev=25, azim=210, paso=8.0, escala=0.3, torre_xy=2.0, alpha=0.85):
...
with st.sidebar:
    ...
    uploaded = st.file_uploader("CSV con columnas: tmax,tmin,viento[,radiacion] (12 filas)", type=["csv"])
...
with col1:
    lat = st.number_input("Latitud", value=float(lat) if lat is not None else center[0], format="%.6f")
with col2:
    lon = st.number_input("Longitud", value=float(lon) if lon is not None else center[1], format="%.6f")
with col3:
    run = st.button("Diseño Bioclimático", type="primary", use_container_width=True)
...
if run:
    ...
    # --- Mostrar 3D ---
    fig3d = plot_modelo_3d(alturas, elev=elev, azim=azim)
    ...
    with col_b:
        ...
        st.pyplot(line_chart(alturas, "Alturas Conceptuales Normalizadas", "%"))
